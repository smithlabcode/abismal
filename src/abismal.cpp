...
namespace nlohmann {
template <typename T> struct adl_serializer<std::atomic<T>> {
  static void
  to_json(json &j, const std::atomic<T> &value) noexcept {
    j = json(value.load());
  }
  static void
  from_json(const json &j, std::atomic<T> &value) {
    T plain_value = j.get<T>();
...
struct ReadLoader {
  ...
  void
  load_reads(std::vector<read_holder> &r) {
    r.clear();
...
struct pe_element {
  ...
  static constexpr std::uint32_t min_dist_default{32};
  static constexpr std::uint32_t max_dist_default{3000};
...
/* GS: this function simply converts the vector<std::uint8_t> pread to a
...
map_single_ended_rand(const bool show_progress, const bool allow_ambig,
  ...
  while (rl) {
    ...
    for (std::size_t i = 0; i < n_reads; ++i) {
      ...
      if (!r[i].read.empty()) {
        // T-rich, + strand
        prep_read<t_rich>(r[i].read, pread_t);
        ...
        // A-rich, + strand
        prep_read<a_rich>(r[i].read, pread_a);
...
map_fragments(const std::uint32_t max_candidates, read_holder &r1,
  ...
  select_maps<swap_ends>(pread1, pread2, r1.cig, r2.cig, res1, res2, mem_scr1,
...
map_paired_ended(const bool show_progress, const bool allow_ambig,
  ...
  std::vector<read_holder> r1;
  std::vector<read_holder> r2;
  ...
  pe_candidates res1;
  pe_candidates res2;
  std::vector<score_t> mem_scr1(std::size(res1.v));
    ...
    for (std::size_t i = 0; i < n_reads; ++i) {
      ...
      if (!bests[i].should_report(allow_ambig)) {
        align_se_candidates(pread1, pread1_rc, pread1, pread1_rc,
                            se_element::valid_frac / 2, res_se1, bests_se1[i],
                            r1[i].cig, aln);

        align_se_candidates(pread2, pread2_rc, pread2, pread2_rc,
                            se_element::valid_frac / 2, res_se2, bests_se2[i],
                            r2[i].cig, aln);
      }
...
struct runner {
  ...
  template <const conversion_type conv>
  void
  paired_ended(const std::string &reads_file1, const std::string &reads_file2,
               const bamxx::bam_header &hdr, bamxx::bam_out &out) {
  ...
  template <const conversion_type conv>
  void
  single_ended(const std::string &reads_file, const bamxx::bam_header &hdr,
               bamxx::bam_out &out) {
...
abismal_make_sam_header(const ChromLookup &cl, const int argc,
  ...
  std::vector<std::size_t> sizes(std::size(names));
  ...
  try {
    ...
    if (verbose)
      log_msg("threads to map reads: " +
              std::to_string(abismal_concurrency::n_threads));
    ...
    runner run{show_progress, allow_ambig, random_pbat, abismal_index};

    if (reads_file2.empty()) {
      if (g_to_a_conversion || pbat_mode)
        run.single_ended<a_rich>(reads_file, hdr, out);
      else if (random_pbat)
        run.single_ended<t_rich>(reads_file, hdr, out);
      else
        run.single_ended<t_rich>(reads_file, hdr, out);
    }
    else {
      if (pbat_mode)
        run.paired_ended<a_rich>(reads_file, reads_file2, hdr, out);
      else if (random_pbat)
        run.paired_ended<t_rich>(reads_file, reads_file2, hdr, out);
      else
        run.paired_ended<t_rich>(reads_file, reads_file2, hdr, out);
    }
